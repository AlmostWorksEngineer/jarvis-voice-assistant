...
# --- FIX: Manually build path to .env file in parent directory ---
...
# Safeguard: if .env fails, use a valid string to prevent crash
if not AssistantVoice:
    AssistantVoice = "en-CA-LiamNeural"

# Asynchronous function to convert text to an audio file
async def TextToAudioFile(text) -> None: #
    # Ensure 'Data' folder exists inside the Backend folder
    ...
    if os.path.exists(file_path): # Check if the file already exists
        try:
            os.remove(file_path) # If it exists, remove it
        except:
            pass
...
def TTS(Text, func=lambda r=None: True): #
    while True: #
        try: #
            ...
            data_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Data", "speech.mp3")
            ...
            break # !!! CRITICAL: Break the loop to stop infinite scrolling
